...
pub fn rune_register_types(module: &mut rune::Module) -> Result<(), rune::ContextError> {
    ...
    module.function_meta(Vec2::splat__meta)?;

    module.function_meta(Vec2::extend__meta)?;

    module.function_meta(Vec2::dot__meta)?;

    module
        .constant("ZERO", Vec2::ZERO)
        .build_associated::<Vec2>()?;

    module.function_meta(Vec2::min__meta)?;
    module.function_meta(Vec2::max__meta)?;
    module.function_meta(Vec2::clamp__meta)?;
    module.function_meta(Vec2::abs__meta)?;
    module.function_meta(Vec2::length__meta)?;
    module.function_meta(Vec2::length_squared__meta)?;
    module.function_meta(Vec2::distance__meta)?;
    module.function_meta(Vec2::floor__meta)?;
    module.function_meta(Vec2::ceil__meta)?;
    module.function_meta(Vec2::fract__meta)?;
    ...
    module.implement_trait::<Vec2>(rune::item!(::std::clone::Clone))?;
...
fn clone_vec(this: &Vec2) -> rune::runtime::VmResult<Vec2> {
    rune::runtime::VmResult::Ok(this.clone())
}

#[rune::function(instance, protocol = DEBUG_FMT)]
fn debug(this: &Vec2, f: &mut rune::runtime::Formatter) -> rune::runtime::VmResult<()> {
    use rune::alloc::fmt::TryWrite;
    rune::vm_write!(f, "{:?}", this)
}

impl rune::runtime::FromConstValue for Vec2 {
    fn from_const_value(
...
impl rune::alloc::prelude::TryClone for Vec2 {
...
fn rune_add(a: Vec2, b: rune::Value) -> Vec2 {
    if let Ok(f) = b.as_float() {
        a + f as f32
    } else if let Ok(vec) = b.downcast::<Vec2>() {
        a + vec
    } else {
        a
    }
...
fn rune_sub(a: Vec2, b: rune::Value) -> Vec2 {
    if let Ok(f) = b.as_float() {
        a - f as f32
    } else if let Ok(vec) = b.downcast::<Vec2>() {
        a - vec
...
fn rune_mul(a: Vec2, b: rune::Value) -> Vec2 {
    if let Ok(f) = b.as_float() {
        a * f as f32
    } else if let Ok(vec) = b.downcast::<Vec2>() {
        a * vec
...
fn rune_div(a: Vec2, b: rune::Value) -> Vec2 {
    if let Ok(f) = b.as_float() {
        a / f as f32
    } else if let Ok(vec) = b.downcast::<Vec2>() {
        a / vec
